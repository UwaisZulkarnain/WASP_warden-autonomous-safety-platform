        ...
        try:
            ...
            for idx, person in enumerate(persons, start=1):
                ...
                for item in ppe_items:
                    ...
                    if item_name in ppe_status and is_inside(item_box, expanded_person_box):
                ...
                if not ppe_status["goggles"]:
                    optional_missing.append("Goggles not detected")
                if not ppe_status["gloves"]:
                    optional_missing.append("Gloves not detected")
                if not ppe_status["boots"]:
                    optional_missing.append("Boots not detected")
            ...
            main_compliance = all(
                p["ppe"]["helmet"] and p["ppe"]["vest"]
                for p in person_outputs
            ) if person_outputs else True
...
<body>
    ...
    <div class="grid">
        ...
        <div style="display: flex; flex-direction: column; gap: 20px;">
            ...
            <div class="panel">
                <h3>Global PPE Status <span style="font-size:12px; color:#94a3b8;">(Helmet & Vest Required)</span></h3>
    ...
    <script>
        async function updateData() {
            try {
                ...
                else if (hasViolation) { statusEl.textContent = 'VIOLATION'; statusEl.style.color = '#ef4444'; }
                ...
                if (c.persons && c.persons.length > 0) {
                    container.innerHTML = c.persons.map(p => {
                        ...
                        const optionalMissing = p.optional_missing && p.optional_missing.length > 0 ? p.optional_missing.join(', ') : 'None';
                        const ppe = p.ppe || {};
                        ...
                        return `<div class="person-card">
                            ...
                            <div class="person-violations">Main Violations: ${violations}</div>
                            <div style="font-size:12px; color:#94a3b8; margin-top:5px;">Optional PPE: ${optionalMissing}</div>
                            <div class="ppe-grid">
                                <div class="ppe-item ${ppe.helmet ? 'ppe-ok' : 'ppe-missing'}">Helmet: ${ppe.helmet ? 'OK' : 'MISS'}<br><span style="font-size:10px;">${conf.helmet ? conf.helmet.toFixed(2) : '0.00'}</span></div>
                                <div class="ppe-item ${ppe.vest ? 'ppe-ok' : 'ppe-missing'}">Vest: ${ppe.vest ? 'OK' : 'MISS'}<br><span style="font-size:10px;">${conf.vest ? conf.vest.toFixed(2) : '0.00'}</span></div>
                                <div class="ppe-item ${ppe.goggles ? 'ppe-ok' : 'ppe-missing'}">Goggles: ${ppe.goggles ? 'OK' : 'MISS'}<br><span style="font-size:10px;">${conf.goggles ? conf.goggles.toFixed(2) : '0.00'}</span></div>
                                <div class="ppe-item ${ppe.gloves ? 'ppe-ok' : 'ppe-missing'}">Gloves: ${ppe.gloves ? 'OK' : 'MISS'}<br><span style="font-size:10px;">${conf.gloves ? conf.gloves.toFixed(2) : '0.00'}</span></div>
                                <div class="ppe-item ${ppe.boots ? 'ppe-ok' : 'ppe-missing'}">Boots: ${ppe.boots ? 'OK' : 'MISS'}<br><span style="font-size:10px;">${conf.boots ? conf.boots.toFixed(2) : '0.00'}</span></div>
                            </div>
                        </div>`;
                    }).join('');
                } else {
